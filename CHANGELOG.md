...
- **Player (Deep Scan & Deduplication):**
    - Updated `play()` to peek inside local playlist files; if a YouTube/HTTP link is found, `mpv` now loads the required `yt-dlp` options (JS runtimes/headers).
    - Introduced `classify_target_weight` helper to centralize URL prioritization rules, removing code duplication between playback methods.
    - Ensures consistent playback behavior for web streams regardless of input method (CLI arg vs Playlist file).
